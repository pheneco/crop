	...
	this.init   = function() {
		if(kind == 'cut') {
			...
			this.render                = function() {
				...
				if(this.square) {if(this.y2 - this.y1 > this.x2 - this.x1) {this.x2 = this.x1 + (this.y2 - this.y1);} else {this.y2 = this.y1 + (this.x2 - this.x1);}}
			...
			this.mousemove      = function(e) {
				var tool        = this.elem.getBoundingClientRect();
				if(this.cutting) {
					this.x2     = e.clientX - tool['left'];
					this.y2     = e.clientY - tool['top'];
					this.render();
				} else if(this.moving) {
