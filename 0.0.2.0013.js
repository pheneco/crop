...
//Written By Trewbot July 10, 2014
...
//		move					event		fired whenever a move action occurs						cut
...
var gra_crop    = function(element, image, kind, options) {
	...
	this.load = function() {
		if(kind == 'cut') {
			this.elem.style.width      = this.img.width;
			this.elem.style.height     = this.img.height;
			this.elem.style.position   = 'relative';
			this.elem.style.background = 'url('+this.img.src+')';
		} else if(kind == 'frame') {
			this.elem.style.width      = options.frame.width;
			this.elem.style.height     = options.frame.height;
			this.elem.style.position   = 'relative';
			this.elem.style.background = 'url('+this.img.src+')';
	...
	this.init   = function() {
		if(kind == 'cut') {
			...
			this.x2                    = options.frame.width;
			this.y2                    = options.frame.height;
			...
			if(this.bleed) {
				var ghost = '<div class="crop-ghost" style="top:-'+this.bleed_width+'px;left:-'+this.bleed_width+'px;width:'+(options.frame.width+(2*this.bleed_width))+'px;height:'+(options.frame.height+(2*this.bleed_width))+'px;'+ ((typeof options.bleed_width == 'number')?'overflow:hidden;':'')+'opacity:'+this.bleed_opacity+';position:absolute;"><img class="crop-ghost-img" src="'+this.img.src+'" style="position:absolute;top:'+(this.y1 + this.bleed_width)+'px;left:'+(this.x1 + this.bleed_width)+'px;"/></div><div style="position:absolute;width:'+options.frame.width+'px;height:'+options.frame.height+'px;top:0;left:0;cursor:move;"></div>';
			...
			this.render                = function() {
				this.elem.style.backgroundPositionX = this.x1 + "px";
				this.elem.style.backgroundPositionY = this.y1 + "px";
				if(this.bleed) {
